      ...
      if (
        ...
        if (target.length) {
          ...
          if ($("html").hasClass("safari")) {
            if (checkWidth(576)) height = 350;
            else if (checkWidth(768)) height = 400;
            else if (checkWidth(992)) height = 450;
            else height = 150;
          } else if ($("html").hasClass("destinations")) {
            if (checkWidth(768)) height = 200;
            else if (checkWidth(992)) height = 250;
            else height = 150;
          }
  ...
  $("#agent-login-form").submit(function(e) {
    ...
    if (!username || username.match(/^ *$/) != null || !password) {
      if (!username || username.match(/^ *$/) != null) {
        $(".wpcf7-not-valid-tip.error-username")
      ...
      if (!password) {
        $(".wpcf7-not-valid-tip.error-password")
          .text("The field is required.")
          .css("display", "block");
      } else {
        $(".wpcf7-not-valid-tip.error-password")
      ...
      $(".wpcf7-response-output.error-all")
...
*/
  ...
  $(".toggle__question").click(function() {
    $(".region-wrapper")
      .not($(this).parents(".region-wrapper"))
      .find(".toggle__answer")
      .slideUp();
    $(".region-wrapper")
      .not($(this).parents(".region-wrapper"))
  ...
  $(".find-more-safaris").click(function() {
    $(this)
      .parent()
      .find(".safari-overflow")
      .slideToggle();
    $(this).text(
      $(this).text() == "View More Safaris"
        ? "View Less Safaris"
        : "View More Safaris"
    );
  });
  ...
  $(".find-safari .filter .checkbox input").change(function() {
    ...
    var destinations = $(".filter .wrapper-checkbox.destination input:checked")
      .map(function() {
        return $(this).val();
      })
      .toArray();

    var styles = $(".filter .wrapper-checkbox.style input:checked")
      .map(function() {
        return $(this).val();
      })
      .toArray();

    var months = $(".filter .wrapper-checkbox.dates input:checked")
      .map(function() {
        return $(this).val();
      })
      .toArray();

    var availability = $(".filter .wrapper-checkbox.availability input:checked")
      .map(function() {
        return {
    ...
    var prices = $(".filter .wrapper-checkbox.price input:checked")
      .map(function() {
        return {
    ...
    var search = $(".your-search .search-block")
      .map(function() {
        return $(this).text();
      })
      .toArray();
    ...
    $(".filter .wrapper-checkbox input").each(function() {
      ...
      if ($(this).prop("checked")) {
        if (!search.includes(text)) {
          var searchBlock = $(
            "<div class='search-block'><div>" + text + "</div></div>"
          )
            .addClass("hidden")
            .appendTo(".your-search");
        ...
        if (search.includes(text)) {
          var searchBlock = $(
            ".your-search .search-block div:contains(" + text + ")"
          ).parent();
    ...
    $(".search-block").click(function() {
      $(".filter .checkbox label:contains(" + $(this).text() + ")")
        .prev()
        .prop("checked", false)
        .change();
    });
    ...
    $(".wrapper-itinerary").each(function() {
      ...
      if (destinations.length > 0) {
        for (var i = 0; i < destinations.length; i++)
          if (itinerary.hasClass(destinations[i])) isDestination = true;
      } else {
        isDestination = true;
      }
      ...
      if (styles.length > 0) {
        for (var i = 0; i < styles.length; i++)
          if (itinerary.hasClass(styles[i])) isStyle = true;
      } else {
        isStyle = true;
      }
        ...
        itinerary.find(".wrapper-tour").each(function() {
          ...
          if (availability.length > 0) {
            ...
            for (var i = 0; i < availability.length; i++) {
              ...
              if (min_value && max_value) {
                ...
                if (places >= min_value) isAvailable = true;
              } else if (max_value) {
                if (places <= max_value) isAvailable = true;
              }
            }
          } else {
            isAvailable = true;
          ...
          if (prices.length > 0) {
            ...
            for (var i = 0; i < prices.length; i++) {
              ...
              if (min_value && max_value) {
                if (cost >= min_value && cost <= max_value) isPrice = true;
              } else if (min_value) {
                if (cost >= min_value) isPrice = true;
              } else if (max_value) {
                if (cost <= max_value) isPrice = true;
              }
            }
          } else {
            isPrice = true;
      ...
      if (show) itinerary.slideDown();
      else itinerary.slideUp();
    });
  });
  ...
  if (
    $("#map-itinerary").length > 0 &&
    JSON.parse($("#map-itinerary").attr("points")) &&
    JSON.parse($("#map-itinerary").attr("config"))
  ) {
  ...
  function loadMapAnimation() {
    ...
    for (var startPoint = 0; startPoint < lengthPoints; startPoint++) {
      ...
      var diffX =
        points[endPoint].geometry.coordinates[0] -
        points[startPoint].geometry.coordinates[0];
      var diffY =
        points[endPoint].geometry.coordinates[1] -
        points[startPoint].geometry.coordinates[1];
      ...
      while (
        (diffX > 0 && i < diffX) ||
        (diffX < 0 && i > diffX) ||
        Math.abs(j) < Math.abs(diffY)
      ) {
        lineCoordinates[startPoint].push([
          points[startPoint].geometry.coordinates[0] + i,
          points[startPoint].geometry.coordinates[1] + j
        ]);
    ...
    function animateLine() {
      if (pointCounter < lengthPoints) {
        if (animationCounter < lineCoordinates[pointCounter].length) {
          geojsonLine.features[0].geometry.coordinates.push(
            lineCoordinates[pointCounter][animationCounter]
          );
  ...
  function checkWidth(pixels) {
    return window.matchMedia(
      "only screen and (max-width: " + pixels + "px)"
    ).matches;
  }
